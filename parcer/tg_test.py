...
def update_user_choice(user_id: int, column: str, value: str):
    try:
        conn = pyodbc.connect(connection_string)
        cursor = conn.cursor()
        cursor.execute(f"UPDATE Users SET {column} = ? WHERE telegramm_id = ?", (value, user_id))
        conn.commit()
        cursor.close()
        conn.close()
    except pyodbc.Error as e:
...
def message_handler(message):
    ...
    if message.text == 'Продолжить':
        ...
        update_user_choice(user_id, 'source_id', message.text)
        on_click1(message)
    elif message.text == 'Подтвердить':
        on_clickCompany(message)
    elif message.text in ['МосБиржа', 'СПБ Биржа', 'Лукойл', 'Газпром', 'Яндекс', 'Вконтакте']:
        update_user_choice(user_id, 'company_id', message.text)
