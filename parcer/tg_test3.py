...
# Проверка, существует ли user_id в базе данных
...
def save_user_choices(user_id: int, source_ids: list, company_ids: list):
    try:
        ...
        for source_id in source_ids:
            print(f"Inserting into Users: user_id={user_id}, source_id={source_id}")
            cursor.execute("INSERT INTO Users (telegramm_id, source_id) VALUES (?, ?)", (user_id, source_id))

        # Вставляем записи в таблицу Users
        for company_id in company_ids:
            print(f"Inserting into Users: user_id={user_id}, company_id={company_id}")
            cursor.execute("INSERT INTO Users (telegramm_id, company_id) VALUES (?, ?)", (user_id, company_id))
